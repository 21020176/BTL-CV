...
def validation(net, val_data_loader, device, exp_name, save_tag=False):
    ...
    for batch_id, val_data in enumerate(val_data_loader):
        ...
        with torch.no_grad():
            ...
            gt = gt.to(device)
            print("afsdfa", input_im, input_im.shape)
        ...
        # --- Save image --- #
        if save_tag:
            save_image(pred_image, image_name, exp_name)

    avr_psnr = sum(psnr_list) / len(psnr_list)
    avr_ssim = sum(ssim_list) / len(ssim_list)
    return avr_psnr, avr_ssim

def infer(net, val_data_loader, device, exp_name, save_tag=False):
    """
    :param net: Gatepred_imageNet
    :param val_data_loader: validation loader
    :param device: The GPU that loads the network
    ...
    input_im, gt, image_name = val_data_loader
    input_im = input_im.to(device)
    gt = gt.to(device)
    input_im = torch.unsqueeze(input_im, 0)
    gt = torch.unsqueeze(gt,0)
    print(input_im, input_im.shape, "afasafsd")
    pred_image= net(input_im)
...
def save_image(pred_image, image_name, exp_name):
    ...
    for ind in range(batch_num):
        ...
        print(image_name_1,"àddsfa")
        print(torch.squeeze(pred_image_images[ind]))
        utils.save_image(torch.squeeze(pred_image_images[ind]), 'tensor_image1.png')
...
def print_log(epoch, num_epochs, one_epoch_time, train_psnr, val_psnr, val_ssim, exp_name):
    ...
    # with open('./training_log/{}_{}_log.txt'.format(exp_name), 'a') as f:
